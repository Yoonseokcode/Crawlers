...
path =os.path.abspath(os.getcwd() + "/geckodriver.exe")      # 드라이버 경로

options = Options()
options.add_argument("--headless")         #headless가 켜져 있을 경우 오류가 좀 더 빈번히 발생하는 경향이 있음
profile = webdriver.FirefoxProfile()

tor=subprocess.Popen(os.path.abspath(os.getcwd() + r'\Tor Browser\Browser\TorBrowser\Tor\tor.exe'))  # 상황에 맞게 수정할 것
...
filepath = os.path.abspath(os.getcwd() + '/EnamineCrawlingResult.xlsm')
...
if os.path.isfile(filepath):
    ...
    excel_sheet1.append(
        ['Name', 'CAS Number', 'Catalog Number', 'Purity', 'Price/Quantity', 'Stock'])  # 엑셀 파일 세팅 집합

driver.get('https://www.enaminestore.com/search')

ECGUI=uic.loadUiType(os.path.abspath(os.getcwd()+'/EnamineCrawlerGUI.ui'))[0]

class WindowClass(QMainWindow,ECGUI):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.message.setText('크롤링 전에 결과 저장 파일이 닫혀 있는지 한번 더 확인해 주세요.')
        self.btn.clicked.connect(self.btnclicked)
        self.lineEdit.returnPressed.connect(self.btnclicked)

    def btnclicked(self):
        self.message.setText('크롤링 진행중...')
        self.message.repaint()

        InputCasNum = self.lineEdit.text()
        if '\n' in InputCasNum:
        ...
        try:
            ...
            if driver.find_element_by_xpath('//*[@id="' + GetID + '_price' + '"]/div[1]').text != 'Indicative prices':
                lenth = len(driver.find_element_by_xpath(
                    '//*[@id="' + GetID + '_price' + '"] / div / div[1] / div / div[1] / div / select').text.splitlines())
            else:
                lenth = len(driver.find_element_by_xpath(
                    '//*[@id="' + GetID + '_price' + '"]/div[2]/select').text.splitlines())  # Quantity / Price에 'Indicative prices'가 존재하는 물질들을 따로 처리하기 위한 부분(ex. 1205513-88-7)
            i = 1

            while i < lenth + 1:
                istr = str(i)
                if driver.find_element_by_xpath(
                        '//*[@id="' + GetID + '_price' + '"]/div[1]').text != 'Indicative prices':
                ...
                if driver.find_element_by_xpath(
                        '//*[@id="' + GetID + '_price' + '"]/div[1]').text != 'Indicative prices':
                    if driver.find_element_by_xpath(
                            '//*[@id="' + GetID + '_price' + '"] / div / div[1] / p').text != 'Out of Stock':
                        Price = driver.find_element_by_xpath(
                            '//*[@id="' + GetID + '_price' + '"]/div/div[1]/div/div[2]/p').text
                    else:
                        Price = ''
                else:
            ...
            self.message.setText("완료!")

        except PermissionError:
            self.message.setText('결과 저장 파일이 열려 있습니다. 파일을 닫고 다시 시도해 주세요.')

        except:
            data = driver.page_source
            soup = BeautifulSoup(data, 'html.parser')
            if soup.select_one(
                    '#content > div:nth-child(5) > p:nth-child(1)').text == 'Nothing was found matching your criteria.':
